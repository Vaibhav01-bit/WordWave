...
export default function Home() {
  ...
  const { articles, trendingArticle, weeklyPopular, loading, likeArticle } = articleContext || {};
  ...
  if (!articleContext || !authContext || authContext.loading || !authContext.isAuthenticated || !likeArticle || !weeklyPopular) {
    return (
  ...
  return (
    <>
      {showIntro && <IntroductoryPage onClose={handleCloseIntro} />}
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              ...
              <aside>
                  {weeklyPopular && <PopularArticles articles={weeklyPopular} />}
              </aside>
          </div>
            ...
            {filteredArticles.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredArticles.map((article: Article) => (
                  <ArticleCard key={article.id} article={article} onLike={likeArticle} />
                ))}
              </div>
            ) : (
